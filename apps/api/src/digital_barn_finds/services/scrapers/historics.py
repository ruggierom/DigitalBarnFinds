...
LOT_PATH_PATTERN = re.compile(r"^/auction/lot/(?P<slug>[^/?#]+)/?$", re.IGNORECASE)
CHASSIS_PATTERN = re.compile(r"Chassis(?:\s+No\.?|\s+Number)?\s*:?\s*(?P<value>[A-Za-z0-9./-]+)", re.IGNORECASE)
...
LOT_NUMBER_PATTERN = re.compile(r"Lot\s+(?P<value>\d+[A-Za-z]?)", re.IGNORECASE)


class HistoricsScraper(BaseScraper):
    source_key = "historics"
    manifest = AdapterManifest(
        ...
        language="en",
        notes="UK auction archive for lot-by-lot public appearance and result tracking.",
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (base_url or settings.historics_base_url).rstrip("/")
        self.seed_paths = seed_paths or settings.historics_seed_paths
        self.delay_seconds = settings.request_delay_seconds if delay_seconds is None else delay_seconds
        self.max_attempts = settings.historics_max_attempts if max_attempts is None else max_attempts
        effective_user_agent = user_agent or settings.effective_user_agent
        self.client = client or httpx.Client(
            headers={"User-Agent": effective_user_agent},
            follow_redirects=True,
            timeout=(
                settings.historics_request_timeout_seconds
    ...
    def parse_discovery_page(self, fixture: FixtureInput) -> list[str]:
        ...
        if fixture.raw_html is None:
            raise ValueError("Historics discovery fixture requires raw_html")
    ...
    def parse_record_fixture(self, fixture: FixtureInput) -> ScrapedCarRecord:
        ...
        if fixture.raw_html is None:
            raise ValueError("Historics detail fixture requires raw_html")
    ...
    def parse_detail_html(
        ...
        *,
        ...
        chassis_number = extract_first_match(CHASSIS_PATTERN, page_text) or self._fallback_serial_number(source_url)
        ...
        lot_number = extract_first_match(LOT_NUMBER_PATTERN, raw_title) or extract_first_match(LOT_NUMBER_PATTERN, page_text)
        ...
        event_name = context.get("sale_title") or "Historics auction appearance"
        ...
        attributes = build_attribute_map(
            {"auction_house": "Historics", "source_heading": self._clean_title(raw_title)},
        ...
        return ScrapedCarRecord(
            ...
            car=NormalizedCar(
                ...
                notes="Parsed from Historics auction lot page.",
    ...
    def _request(self, method: str, url: str) -> httpx.Response:
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                last_error = exc
                self._log(f"Historics request failed ({attempt}/{self.max_attempts}) for {url}: {exc}")
                if attempt < self.max_attempts:
                    time.sleep(self.delay_seconds)
        if last_error is None:
            raise RuntimeError(f"Historics request failed with no exception for {url}")
        raise last_error
    ...
    def _clean_title(self, title: str) -> str:
        cleaned = re.sub(r"^Lot\s+\d+\s*-\s*", "", title, flags=re.IGNORECASE)
        cleaned = re.sub(r"\s+\|\s+Historics.*$", "", cleaned, flags=re.IGNORECASE)
    ...
    def _extract_context(self, soup: BeautifulSoup, page_text: str) -> dict[str, str]:
        ...
        if not sale_title:
            anchor = soup.find("a", href=lambda value: value and "/auction/details/" in value)
            if anchor:
                sale_title = normalize_space(anchor.get_text(" ", strip=True))
        if sale_title and not sale_date:
            sale_date = sale_title
        if sale_title and not sale_location:
            sale_location = sale_title.split(",", 1)[0]
        return {
    ...
    def _fallback_serial_number(self, source_url: str) -> str:
        return urlparse(source_url).path.strip("/").replace("/", "-") or "historics-unknown"
